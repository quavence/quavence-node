...
struct CStakeCache{
    CStakeCache(uint32_t blockFromTime_, CAmount amount_, const CScript& scriptPubKey_ = CScript())
        : blockFromTime(blockFromTime_), amount(amount_), scriptPubKey(scriptPubKey_) {
    }
...
bool CheckStakeKernelHash(const CBlockIndex* pindexPrev, unsigned int nBits, uint32_t blockFromTime, CAmount prevoutValue, const COutPoint& prevout, const CScript& prevoutScript, unsigned int nTimeTx, bool fPrintProofOfStake = false);
