...
BOOST_AUTO_TEST_CASE(test_addresses)
{
    ...
    for (size_t i = 0; i < hash.size(); ++i)
    {
        ...
        std::string encKey = EncodeCashAddr(dstKey, params);
        BOOST_CHECK_MESSAGE(!encKey.empty(), "EncodeCashAddr returned empty for key " + std::to_string(i));
        BOOST_CHECK_MESSAGE(encKey.find(params.CashAddrPrefix()) == 0, "Wrong prefix in " + encKey);

        const CTxDestination dstScript = CScriptID(uint160(hash[i]));
        std::string encScript = EncodeCashAddr(dstScript, params);
        BOOST_CHECK_MESSAGE(!encScript.empty(), "EncodeCashAddr returned empty for script " + std::to_string(i));
        BOOST_CHECK_MESSAGE(encScript.find(params.CashAddrPrefix()) == 0, "Wrong prefix in " + encScript);
    }
}
